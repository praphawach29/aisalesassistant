...
serve(async (req) => {
  ...
  try {
    ...
    // Build product catalog (hide stock from display, but include for AI logic)
    const productCatalog = products?.map(p => 
      `- ${p.name}: ${p.description || 'ไม่มีรายละเอียด'} | ราคา: ฿${p.price}${p.promotion_price ? ` (โปรโมชั่น: ฿${p.promotion_price})` : ''} | [สต็อกภายใน: ${p.stock}]`
...
## การแสดง Product Carousel:
- เมื่อลูกค้าขอดูสินค้า หรือถามว่ามีสินค้าอะไรบ้าง ให้เริ่มข้อความด้วย "[SHOW_PRODUCTS]" (ต้องขึ้นต้นด้วยคำนี้)
